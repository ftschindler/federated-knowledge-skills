<!--
  ...
  Pinned from the canonical repository. Note that the validator shipped beside
  this file (../scripts/okf_validate.py) implements an earlier revision: it
  expects `stale_after`, `sources[].last_modified` and `usage_window` as
  `YYYY-MM-DD` dates rather than the ISO 8601 datetimes specified below, and
  rejects the datetime form. Write dates.
-->
