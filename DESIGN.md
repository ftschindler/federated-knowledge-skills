...
### 9.8 Who maintains `index.md`, and what its entries cost

OKF §8 says an index entry SHOULD carry the description from the linked concept's
frontmatter, and §5.1 makes `index.md` the first thing a reading agent opens. Both are
reasons to have one. Neither says who writes it.

Written by the filing agent (§6.3 step 6), the concept's `description` lands in two files at
once and can drift - the objection that removed `topic:` in §9.6. Generated by `fkb`, it
cannot drift, but `fkb` acquires a writing mode: §7 currently promises it "never touches a
concept body, renders, or hashes", and an index is derived from frontmatter rather than a
concept body, so the promise bends rather than breaks. Invariant 1 is untouched either way -
the index is not a shadow of a knowledge file, it is a table of contents over many.

A third answer is available and cheap: OKF §8 says a consumer MAY synthesize an index when
none is present, so the file could simply not exist and `fkb list`-style output could stand
in. That trades the published site's index page away, which is a real loss for a bundle that
is also a website.

**Decide when the index stops being maintainable by hand** - roughly the same moment §9.1
starts mattering, and probably from the same evidence. Until then the filing agent writes it,
and the journal should record any occasion where the index and a concept disagreed.
